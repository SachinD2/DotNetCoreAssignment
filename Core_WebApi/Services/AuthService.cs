...
namespace Core_WebApi.Services
{
	...
	public class AuthService
	{
		...
		public AuthService(IConfiguration config,SignInManager<IdentityUser> sign, UserManager<IdentityUser> use)
		{
			userManager = use;
			signInManager = sign;
	...
	/*	public async Task Invoke(HttpContext httpContext)
		{
			...
			await _context.MiddlewareClasses.AddAsync(new MiddlewareClass()
			{
				LogGUID = httpcontext.Request.Headers["JwtSecurityToken"],
				...
				Exception = httpcontext.TraceIdentifier.ToLower()
			}) ;
		...
		public async Task<bool> RegisterUser(RegisterUser user)
		{
            ...
            {
				// await _auth.Invoke(httpcontext);
            }
			return false;
		}
